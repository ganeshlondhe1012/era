...
- Document attachment
- Text extraction
- PDF support for the existing document-attachment feature
- Foundation for document understanding

### Retrieval-Augmented Generation (RAG)

Era uses a separate local Python RAG service. Flutter remains the desktop UI, while
Python performs document extraction, chunking, embeddings, vector persistence,
retrieval, prompt augmentation, and generation.

The MVP supports TXT/Markdown files and uses Ollama `nomic-embed-text` for embeddings,
SQLite for local vector persistence, cosine similarity for retrieval, and the selected
Ollama chat model for final generation.
...
For the RAG MVP, also download the embedding model:
...
The RAG backend lives in `python_rag/` and is a local FastAPI service on
`http://127.0.0.1:8787`.
...
Python FastAPI RAG service
      ...
      `-- context augmentation + Ollama generation
```
...
```text
python_rag/start_rag_service.bat
```
...
Start Era after the service is running. The Flutter app sends document files and
RAG questions to the Python service. Normal Era chat continues to use the existing
Ollama provider directly.

### Verification status

The Python backend source passes Python syntax checks and has unit tests for the
core chunking and cosine-similarity logic. A full Flutter build and live Ollama
end-to-end test still require the corresponding runtimes to be installed.
...
- RAG currently supports TXT/Markdown ingestion, local Ollama embeddings, local vector persistence, and cosine-similarity retrieval.
...
GitHub Actions runs Python syntax/unit tests plus Flutter dependency installation, Dart analysis, Flutter tests, and a Windows release build on pushes and pull requests.

A live Ollama/RAG end-to-end test is intentionally not part of CI because Ollama models are large and the production app is designed for local inference.
