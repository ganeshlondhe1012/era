...
class DocumentService {
  DocumentService({required this._repository});
  ...
  Future<void> initialize() async {
    _documents
      ..clear()
      ..addAll(await _repository.loadDocuments());
  }

  Future<void> addDocument(Document document) async {
    _documents.removeWhere(
      (existing) => existing.id != document.id && existing.path == document.path,
    );
  ...
  List<Document> search(String query) {
    ...
    return _documents.where((document) {
      return document.name.toLowerCase().contains(q) ||
          document.path.toLowerCase().contains(q);
    }).toList();
  }
