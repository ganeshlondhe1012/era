...
class ImportDocumentService {
  ImportDocumentService({required this._pipeline, required this._extractors});
  ...
  Future<void> importDocument(Document document) async {
    ...
    final text = await extractor.extract(document);

    if (text.trim().isEmpty) {
      throw Exception('Document contains no readable text.');
    }

    await _pipeline.indexDocument(document: document, text: text);
  }
  ...
  List<String> supportedExtensions() {
    return _extractors.map((extractor) {
      if (extractor.runtimeType.toString().contains('Txt')) {
        return '.txt';
      }

      if (extractor.runtimeType.toString().contains('Markdown')) {
        return '.md';
      }
  ...
  TextExtractor? _findExtractor(Document document) {
