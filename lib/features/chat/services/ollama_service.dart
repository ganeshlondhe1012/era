...
''');
  ...
  /// Generates a local embedding using Ollama's embedding API.
  Future<List<double>> generateEmbedding({
    required String text,
    required String model,
  }) async {
    final uri = Uri.http('$host:$port', '/api/embed');
...
Make sure the embedding model is installed, for example:
ollama pull $model
''');
    }
    ...
    if (embeddings is! List || embeddings.isEmpty || embeddings.first is! List) {
      throw Exception('Ollama returned no embedding vector.');
    }

    return List<double>.from(
      (embeddings.first as List).map((value) => (value as num).toDouble()),
    );
  }
